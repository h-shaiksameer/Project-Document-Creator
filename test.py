...
@login_required
def approve_users():
    if current_user.email != "admin_email@example.com":  # Check if the user is admin
        return redirect(url_for('home_page'))  # Redirect non-admins to the home page
    ...
    try:
        ...
        # Create a list of user objects with 'name', 'email', 'approved' and 'id'
        pending_users = []
        if pending_users_data:
            for user_id, user_info in pending_users_data.items():
                user = {
                    'id': user_id,
                    'name': user_info.get('name'),
                    'email': user_info.get('email'),
                    'approved': user_info.get('approved')
                }
